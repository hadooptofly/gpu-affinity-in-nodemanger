...
public class ContainerLaunch implements Callable<Integer> {
  ...
  public void cleanupContainer() throws IOException {
    ...
    try {
      ...
      // kill process
      if (processId != null) {
        Class<? extends ResourceCalculatorProcessTree> processTreeClass = conf.getClass(YarnConfiguration.NM_CONTAINER_MON_PROCESS_TREE, null,
            ResourceCalculatorProcessTree.class);
        ResourceCalculatorProcessTree pTree =
            ResourceCalculatorProcessTree.getResourceCalculatorProcessTree(processId, processTreeClass, conf);
        pTree.updateProcessTree();    // update process-tree
        String allTaskPids = pTree.getAllTaskPids();
        String relativation = pTree.getAllRelativeTuple();
        if(org.apache.commons.lang.StringUtils.isBlank(allTaskPids) || allTaskPids.equals("-1")){
          allTaskPids = processId;
        }
        ...
        if (sleepDelayBeforeSigKill > 0) {
          ...
          allTaskPids, relativation, sleepDelayBeforeSigKill, Signal.KILL, exec).start();
        }
      }
    } catch (Exception e) {
      String message =
